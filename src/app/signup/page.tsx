...
const formSchema = z.object({
  ...
  altchaPayload: z.string({ required_error: 'Veuillez résoudre le captcha.' }).min(1, 'Veuillez résoudre le captcha.'),
...
export default function SignupPage() {
  ...
  const altchaWidgetRef = useRef<HTMLElement>(null);
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    if (result.success) {
        ...
        // Reset Altcha widget for another attempt
        altchaWidgetRef.current?.dispatchEvent(new CustomEvent('reset'));
    }
  };
        ...
        <CardContent>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
             ...
             <div className="grid gap-2 p-4 border rounded-md">
                 ...
                 <Controller
                    ...
                    render={({ field }) => (
                        <altcha-widget
                            ref={altchaWidgetRef}
                            challengeurl="https://altcha.dev/api/v1/challenge"
                            onStateChange={(e: any) => {
                                const state = e.detail;
                                if (state.state === 'verified') {
                                    field.onChange(state.payload);
                                }
                            }}
                        ></altcha-widget>
                    )}
                 />
                 {form.formState.errors.altchaPayload && <p className="text-sm text-destructive">{form.formState.errors.altchaPayload.message}</p>}
             </div>
            ...
            <Button type="submit" className="w-full" variant="destructive" disabled={form.formState.isSubmitting}>
                {form.formState.isSubmitting ? 'Création en cours...' : 'Créer mon compte'}
            </Button>
