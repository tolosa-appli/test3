...
import { jobSectors } from './sectors-data';

// Schemas for forms
const offerSchema = z.object({
    employerName: z.string().min(2, "Le nom de l'employeur doit contenir au moins 2 caractères."),
    employerAddress: z.string().min(3, "L'adresse doit contenir au moins 3 caractères."),
    employerCity: z.string().min(2, "La ville doit contenir au moins 2 caractères."),
...
const JobOfferForm = () => {
    ...
    const onSubmit = (data: OfferFormData) => {
        console.log("New job offer submitted:", data);
        toast({ title: "òsca !", description: "Offre d'emploi publiée avec succès." });
        reset();
    };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <Input id="employerName" placeholder="Ex: Tech Innovate" {...register('employerName')} />
                     ...
                     <Controller name="industry" control={control} render={({ field }) => (
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                            ...
                            <SelectContent className="max-h-[400px]">
                                {jobSectors.map(category => (
                                    <SelectGroup key={category.name}>
                                        <SelectLabel>{category.name}</SelectLabel>
                                        {category.subcategories.map((sub, index) => (
                                            typeof sub === 'string' ? 
                                                <SelectItem key={`${category.name}-${sub}`} value={sub}>{sub}</SelectItem> :
                                                <SelectGroup key={`${category.name}-${sub.name}`}>
                                                    <SelectItem value={sub.name} className="font-semibold">{sub.name}</SelectItem>
                                                    {sub.subcategories.map(s => (
                                                         typeof s === 'string' ?
                                                            <SelectItem key={s} value={s} className="pl-8">{s}</SelectItem> :
                                                            <SelectGroup key={s.name}>
                                                                <SelectItem value={s.name} className="pl-8 font-semibold">{s.name}</SelectItem>
                                                                {s.subcategories.map(ss => <SelectItem key={ss} value={ss} className="pl-12">{ss}</SelectItem>)}
                                                            </SelectGroup>
                                                    ))}
                                                </SelectGroup>
                                        ))}
            ...
            <div className="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <Input id="employerAddress" placeholder="Ex: 1 Rue de l'Innovation" {...register('employerAddress')} />
                    {errors.employerAddress && <p className="text-sm text-destructive">{errors.employerAddress.message}</p>}
                </div>
                <div>
                    <Label htmlFor="employerCity">Ville</Label>
                    <Input id="employerCity" placeholder="Ex: Toulouse" {...register('employerCity')} />
                    {errors.employerCity && <p className="text-sm text-destructive">{errors.employerCity.message}</p>}
                </div>
            </div>

            <div>
                <Label htmlFor="positionTitle">Nom du poste</Label>
                <Input id="positionTitle" placeholder="Ex: Développeur Full-Stack" {...register('positionTitle')} />
            ...
            <div className="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <Input id="experienceRequired" placeholder="Ex: 3 ans" {...register('experienceRequired')} />
                ...
                <div>
                    ...
                    <Input id="educationLevel" placeholder="Ex: Bac +5" {...register('educationLevel')} />
...
const JobRequestForm = () => {
    ...
    const onSubmit = (data: RequestFormData) => {
        console.log("New job request submitted:", data);
        toast({ title: "òsca !", description: "Demande d'emploi publiée avec succès." });
        reset();
    };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
             ...
             <div>
                ...
                <Input id="positionSought" placeholder="Ex: Chef de projet digital" {...register('positionSought')} />
                {errors.positionSought && <p className="text-sm text-destructive">{errors.positionSought.message}</p>}
            </div>
            
            <div className="grid md:grid-cols-2 gap-4">
                <div>
                    <Label htmlFor="diploma">Dernier diplôme acquis</Label>
                    <Input id="diploma" placeholder="Ex: Master Marketing" {...register('diploma')} />
                    {errors.diploma && <p className="text-sm text-destructive">{errors.diploma.message}</p>}
                </div>
                <div>
                    <Label htmlFor="training">Votre formation</Label>
                    <Input id="training" placeholder="Ex: Digital School" {...register('training')} />
        ...
        <div className="container mx-auto p-4 md:p-8">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Trouvez votre prochain emploi ou le candidat idéal.
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-8">
                    <Tabs defaultValue="offer" className="w-full">
                        <TabsList className="grid w-full grid-cols-2">
                            <TabsTrigger value="offer">Je propose un emploi</TabsTrigger>
                            <TabsTrigger value="request">Je cherche un emploi</TabsTrigger>
                        </TabsList>
                        ...
                        <TabsContent value="request">
                             <Card>
                                <CardHeader>
                                    <CardTitle>Nouvelle Recherche d'Emploi</CardTitle>
                    ...
                    <div>
                        <h2 className="text-2xl font-bold font-headline my-4">Annonces d'emploi</h2>
