...
import { adsData, adCategories } from './data';
import { adImageMapping } from './image-map';
...
const AdForm = ({ setDialogOpen }: { setDialogOpen: (open: boolean) => void }) => {
    ...
    const { register, handleSubmit, control, watch, formState: { errors }, reset } = useForm<AdFormData>({
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                ...
                                <SelectContent>
                                    {Object.keys(adCategories).map(cat => <SelectItem key={cat} value={cat}>{cat}</SelectItem>)}
                                </SelectContent>
                            </Select>
                        )}
                    />
                     ...
                     <Controller
                        ...
                        render={({ field }) => (
                            <Select onValueChange={field.onChange} defaultValue={field.value} disabled={!selectedCategory}>
                                <SelectTrigger><SelectValue placeholder={selectedCategory ? "Sélectionnez..." : "Choisissez d'abord une catégorie"} /></SelectTrigger>
                                <SelectContent>
                                    {selectedCategory && adCategories[selectedCategory].map(sub => <SelectItem key={sub} value={sub}>{sub}</SelectItem>)}
                                </SelectContent>
                            </Select>
                        )}
                    />
...
export default function AdsPage() {
    ...
    const getAdImage = (ad: typeof adsData[0]) => {
        const categoryMapping = adImageMapping[ad.category];
        if (categoryMapping) {
            return categoryMapping[ad.subcategory] || categoryMapping.default || ad.image;
        }
        return ad.image;
    }
    ...
    const displayAds = (searchTerm.length < 3 && categoryFilter === 'all') ? adsData : filteredAndSortedAds;
    const showEmptySearchMessage = (searchTerm.length >= 3 || categoryFilter !== 'all') && filteredAndSortedAds.length === 0;
            ...
            <Card>
                ...
                <CardContent className="space-y-8">
                    ...
                    <div>
                        ...
                        {searchTerm.length > 0 && searchTerm.length < 3 && categoryFilter === 'all' && (
                            <div className="text-center text-destructive py-10">
                                <p>mot trop court, trois lettres minimum !</p>
                            </div>
                        )}

                        {(searchTerm.length >= 3 || categoryFilter !== 'all') && filteredAndSortedAds.length > 0 && (
                            ...
                            {displayAds.map(ad => (
                                <Card key={ad.id} className="overflow-hidden flex flex-col group">
                                     <div className="relative h-48 w-full">
                                        <Image src={getAdImage(ad)} alt={ad.title} layout="fill" objectFit="cover" data-ai-hint={ad.dataAiHint} />
